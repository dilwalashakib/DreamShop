    ...
    return (
        <div className='relative overflow-x-auto'>
           <table className='w-full text-sm text-left'>
                ...
                <tbody>
                    {data?.map((item, i) => (
                        <tr key={i} className='bg-black border-b border-gray-700'>
                            ...
                            <td scope='row' className='whitespace-nowrap px-4 py-3'>
                                <Link href={`/seller/order-details?id=${item?._id}`} className="bg-green-600 hover:bg-green-700 py-1 px-2 rounded-md text-white uppercase">
                                    View
                                </Link>
                            </td>
                        </tr>
